...
def cycle(link):
    ...
    allVisited = []
    linkz.append(link)
    allVisited.append(link)
    thridPartyLinks = []
    url = getUrlName(link)
    count = 0
    #The top has nothing to do with the bottem.
    toAppend = []
    
    while len(linkz) > 0: # While the program is still going
        count = count + 1
        ...
        #Only wory about stuff past this point:



        print "    -" + linkz[0]
        writeToFile("links.txt", "    -" + linkz[0])
        bro.open(linkz[0], timeout=5.0)
        for i in toAppend:
            print "[Appended] | " + i
            linkz.append(i)
        
        for lnk in bro.links(): #Cycle through the links
            ...
            if url in finalLink:
                if finalLink not in allVisited:
                    print "        ~" + str(finalLink)
                    writeToFile("links.txt", "        ~" + str(finalLink))
                    linkz.pop(0)
        ...
        for link in linkz:
            if url in link:
                if link not in allVisited:
                    print "        =~" + str(finalLink)
                    writeToFile("links.txt", "        =~" + str(finalLink))
                    linkz.pop(0)
                    bro.open(link, timeout=5.0)
                    for t in bro.links():
                        toAppend.append(t)
                else:
                    pass
...
def urlScraper(url, scrape, savePgs):
    ...
    print "Currently Crawling [" + url + "] (Please allow a while for me to get done)"
    print "-----"*20
    writeToFile("links.txt", text)
    urls = [url]
    visited = [url]
    retry = 0
                    ...
                    if newurl not in visited and url in newurl: #if its not been looked at and if the url is from the site im scraping
                        ...
                        for adminDetection in adminDetections:
                            if adminDetection in newurl.lower():
                                ...
                                writeToFile("adminlinks.txt", newurl)
                        ...
                        if ".pdf" in newurl:
                            ...
                            writeToFile(slocation + "savedPDF" + str(saveNum) + ".pdf", pdfContent)
                            saveNum = saveNum + 1
                            
                        if "mailto:" in newurl:
                            print "[Email] | > " + newurl
                            writeToFile("scrapedEmails.txt", newurl)
            ...
            for passw in password:
                ...
                for word in response.split():
                    if word.strip() in successTestFile.readlines():
                        ...
                        writeToFile("savedAccounts.txt", str(user) + "  | " + str(passw) + " | " + str(url))
                    else:
                        pass
        except KeyboardInterrupt:
            print """
...
def scrapeURL(url, saveResults, findURL, findEmail):
    ...
    def visible(element):
        ...
        try:
            ...
            if saveResults == True:
                try:
                    writeToFile(slocation + "URLScrape.txt", new)
                except:
                    pass
...
def scraperChoose(url, scrapetype):
    color("a")
    scrapetype = raw_input("""Paradox Scraper:
/-------------------------------------\\
|URL Spider>---------------(1)        |
|Webpage Scraper>----------(2)        |
|Delete Logs>--------------(3)        |
