...
def sendMessage(msg):
    ...
    server.starttls()
    server.login( 'east837@gmail.com', '*****' )
...
def sellProfit(Type):
    ...
    if "phone" in Type:
        ...
        return "\n> Monitoring of [" + shareName + "] Paused. (Details Below)\n>" + "---"*16 + "<\n[Buy Price] | $" + str(formNum(totalBuyPrice)) + "\n[Sell Price(Now)] | $" +  str(formNum(totalSellPrice)) + "\n[Total Sell Profit] | $" + str(formNum(moneyEarned))
    else:
        print "[Error] | > Unknown: sellProfit(Type)"
...
def sellShares(Amount):
    ...
    totalBuyPrice = round(float(buyPrice) * float(Amount),2)
    totalSellPrice = round(float(currentPrice) * float(Amount),2)
    moneyEarned = totalSellPrice - totalBuyPrice
    oldBallance = currentBallance
    currentBallance = currentBallance + moneyEarned
    shareAmount = shareAmount - Amount
    print "[Broker] | NewShareAmount: " + str(shareAmount-Amount) + " | Old Ballance: " +  str(formNum(oldBallance)) + " | New Ballance: " + str(formNum(currentBallance)) + " | Money Differance: " + str(formNum(moneyEarned))
    if shareAmount <= 0:
        findStocksToBuy()
    return
